...
var QidLS = [] // ÁTMENETILEG HASZNÁLOM CSAK Qtxt to LSid
...
function F_getMissQs(){
	...
	var missIDs = ""
	var normIDs = ""
	for ( var LSid=0; LSid<localStorage.getItem("hkQ.max"); LSid++ ) {
...
function F_checkEXPs(){
	for ( var i = 0; i < elems.length; i++ ) {
		...
		if ( elem.className.indexOf("{") > -1 ) {
			...
			var txtDef = '<details class="' +elem.className+ '">' +elem.innerHTML+ "</details>"
			txt = LZString.compressToUTF16(txtDef)
			var LSid = false
...
//			if ( EXPid == "173" ) { alert(EXPid+": "+LSid+": "+txtDef) }
			LStxt[LSid] = txtDef
			txtLS[txtDef] = LSid
			//alert(LSid + " : " + txtDef)
			//alert(LSid + " " + txtDef)
...
function F_checkQs(){
	...
	for ( var i=0; i<kerdesek.length; i++ ) { 
		...
		var LSid = ""
		if ( Qelem.className.indexOf("{") == -1 ) { // nem {EXP}
			if ( arrOLDtxt[Qtext] ) {
				LSid = arrOLDtxt[Qtext]
			...
			if ( LSid != "" ) { 
				var Qtext = LZString.compressToUTF16(Qtext);
	...
	if ( LSid == "hkQ.3574" ) { alert("sajt") }
			...
			// if ( Qtext.slice(begin2,end2) == "kamraizomzat Ca<sup>2+</sup>-anyagcseréje(felszabadulása/eltűntetése)" ) { 
			// 	alert("AFT:"+LSid+": "+Qtext.slice(begin2,end2)) 
			// }
		}
	}
	
	
	/*alert("1356: "+LStxt["hkQ.1356"])
	//console.clear()
	for ( var Qtext in arrOLDtxt ) {
		var begin = Qtext.indexOf("<summary>")
		var end = Qtext.indexOf("</summary>")
		console.log(arrOLDtxt[Qtext]+": "+Qtext.slice(begin+9,end))
	}
	console.log("––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––")
	console.log("––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––")
	console.log("––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––")
	for ( var LSid in arrNEWid ) {
		var begin = arrNEWid[LSid].indexOf("<summary>")
		var end = arrNEWid[LSid].indexOf("</summary>")
		console.log(LSid+": "+arrNEWid[LSid].slice(begin+9,end))
	}
	alert("stop")*/
		...
		document.getElementById("button_replaceQ").onclick = function() {
			...
			function changeLS(type){
				if ( localStorage.getItem(oldLSid+type) ) {
					//alert(oldLSid+","+type+": "+localStorage.getItem(oldLSid+type))
					localStorage.setItem(value+type,localStorage.getItem(oldLSid+type))
					localStorage.removeItem(oldLSid+type)
					//console.log(value+type+": " +localStorage.getItem(value+type))
				}
			}
			changeLS("_jegy")
			changeLS("_skip")
...
function F_oldQcheck(){
	...
	for ( var Qtext in arrOLDtxt ) {
		var begin = Qtext.indexOf("<summary>")
		var end = Qtext.indexOf("</summary>")
		console.log(arrOLDtxt[Qtext]+" : "+localStorage.getItem(arrOLDtxt[Qtext]+'_idopont')+" : "+Qtext.slice(begin+9,end))
	}
	console.log("––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––")
	alert("STOP")*/
	
	var size = Object.keys(arrOLDtxt).length;
	if ( size > 0 ) {
		for (var oldQtxt in arrOLDtxt) {
		...
		localStorage.setItem("hkQ.miss",missQs.toString())
	}
}
	...
	for ( var i=0; i<impek.length; i++ ) {
		...
		function F_impQ(EXPid){
			if ( localStorage.getItem("hkExpQ."+EXPid) ) { // kell, különben, ha egy hiányzik, akkor nem tölti be az egészet
				...
				var LSid = string.slice(0,string.indexOf(" "))
				string = string.slice(string.indexOf(" ")+1)
				var IMGloc = string.slice(0,string.indexOf(" ## "))
				var Qtext = string.slice(string.indexOf(" ## ")+4)
				
				Qtext = LZString.decompressFromUTF16(Qtext)
				//alert(LSid + " : " + IMGloc + " : " + Qtext)
				//console.log("EXPid: "+EXPid)
				//console.log("Qtext: "+Qtext)
				...
				if ( Qtext == "undefined" ) { 
					...
					if ( Qelem.innerHTML.indexOf("{"+checkID+"}") == -1 && Qelem.className.indexOf("{"+checkID+"}") == -1 ) { 
						...
						for ( var x=0; x<imgs.length; x++ ) {
							imgs[x].dataset.src =  htmlLEARNloc + IMGloc + imgs[x].dataset.src
						}*/
					}
				}
			} else {
				alert("az alábbi EXPid-s questet nincs elmentve LS-be: "+EXPid)
			}
		}
...
function func_tableSkipFix(){
	for ( var i=0;  i<kerdesek.length;  i++ ) {
		...
		if ( kerdesek[i].id ) { 
			...
			Qtext = '<details class="' +kerdesek[i].className+ '">' +kerdesek[i].innerHTML+ "</details>"
		}
...
function F_nextQ(){
	...
	if ( priorQid != "nincs" ) {
		...
		function func_setTitle(){
			...
			var xQtxt = arrQid[priorQid] // Qid to Qtxt
			var xLSid = txtLS[xQtxt] // Qtxt to LSid
			document.getElementById("questLeveL").title = xLSid
		}
		func_setTitle()
		...
		function F_copyQs(Qelem){
			...
			func_addQ(Qtext)
			if ( Qelem.id ) { 
				var txt = arrQid[Qelem.id] // Qid to Qtxt
				var LSid = txtLS[txt] // Qtxt to LSid
				QidLS[Qtext] = LSid
			}
